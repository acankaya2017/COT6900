...
# To preview the raw gene counts let's look at the expression levels of the first 6 genes in the first 10 samples...
...
#   reference_group - indicates which of the condition variable values is the reference group (no tumors)
...
#   topTable - a simple table which contains the top 100 differentially expressed genes sorted by p.value
limma_pipeline = function(
  ...
  if (!is.null(reference_group)) {
    group = relevel(group, ref=reference_group) # otherwise just pick the first class as the reference class
  }
  ...
  dge = dge[keep,,keep.lib.sizes=FALSE] # filter the DGEList object and only keep the genes we want
  ...
  # use the voom method to conver the data to have a similar variance as arrays - as required by limma
...
Step 3 - Visualize
```{r}
# make a function to generate a scatter plot to show a separation of tumor vs normal classification points
...
# do an elastic net model - a generalized linear model that combines lasso and ridge regression,
#  it selects the genes or groups of genes that best predict the condition and uses these to build the model
...
# the res object here is an object that holds the model coeffiecients and the mean error found during training
res = cv.glmnet(
  x = x_train,
  y = y_train,
  alpha = 0.5,
